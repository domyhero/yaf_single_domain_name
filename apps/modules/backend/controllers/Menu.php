...
class MenuController extends \common\controllers\Admin {
    ...
    public function addAction() {
        if ($this->_request->isPost()) {
            ...
            $ctrl_name = $this->getString('c');
            $a_name    = $this->getString('a');
            $data      = $this->getString('data', '');
            $listorder = $this->getInt('listorder', 0);
            $display   = $this->getInt('display', 0);
            $ok = AdminPermissionService::addMenu($parentid, $name, $ctrl_name, $a_name, $data, $listorder, $display);
            if ($ok) {
                $this->json($ok, '添加成功');
            } else {
                $this->json($ok, '添加失败');
    ...
    public function editAction() {
        if ($this->_request->isPost()) {
            ...
            $display         = $this->getInt('display', 0);
            $ok = AdminPermissionService::editMenu($menu_id, $parentid, $name, $controller_name, $action_name, $data, $listorder, $display);
            if ($ok) {
                $this->json($ok, '编辑成功');
            } else {
                $this->json($ok, '编辑失败');

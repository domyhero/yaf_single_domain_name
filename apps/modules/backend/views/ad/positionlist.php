...
<div class="subnav">
	<div class="content-menu ib-a blue line-x">
		<a class="add fb"
			href="javascript:postDialog('addAdPostion', '<?php echo YUrl::createBackendUrl('Ad', 'positionAdd'); ?>', '添加广告位置', 400, 150)"><em>添加广告位置</em></a>
						...
						<div class="explain-col">
							<p style="margin-top: 10px;">
								<input type="text" value="<?php echo $keywords; ?>"
									class="input-text" name="keywords" placeholder="广告位置名称" /> <input
    ...
    <?php foreach ($list as $item): ?>
    	<tr>
						...
						<td align="center"><a href="###"
							onclick="edit(<?php echo $item['pos_id'] ?>, '<?php echo $item['pos_name'] ?>')"
							title="修改">修改</a> | <a href="###"
							onclick="deleteDialog('positionDelete', '<?php echo YUrl::createBackendUrl('Ad', 'positionDelete', ['pos_id' => $item['pos_id']]); ?>', '<?php echo $item['pos_name'] ?>')"
							title="删除">删除</a> | <a href="###"
							onclick="setAdPosValue(<?php echo $item['pos_id'] ?>, '<?php echo $item['pos_name'] ?>')"
							title="广告管理">广告管理</a></td>
					</tr>
    <?php endforeach; ?>
    </tbody>
			</table>
